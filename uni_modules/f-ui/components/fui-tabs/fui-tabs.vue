<template>
	<view class="fui-tabs">
		<view class="fui-tabs__wrap" :style="[styles, lStyle]">
			<slot name="left"></slot>
			<scroll-view class="fui-tabs__scroll" 
				:class="{'fui-tabs__scroll--split': split}"
				enhanced 
				enable-flex 
				...
				@scroll="onScroll">
				<view class="fui-tabs__nav">
					<view class="fui-tabs__item" 
						:style="{color: !item.disabled && (index == currentIndex ? activeColor : color)}"
						:class="[
							'fui-tabs__item-text--' + size, 
							{
								
								'fui-tabs__item--active': index == currentIndex, 
								'fui-tabs__item--evenly': spaceEvenly, 
								'fui-tabs__item--disabled': item.disabled
							}
						]" 
						...
						<slot name="label" :item="item" :active="index == currentIndex" :disabled="item.disabled">
							<fui-badge
								...
								:offset="item.offset"
								:content="item.badge">
								<view class="fui-tabs__item-text l-ellipsis">
									{{item.label}}
								</view>
							</fui-badge>
							<view v-else class="fui-tabs__item-text l-ellipsis">
								{{item.label}}
							</view>
						</slot>
						
					</view>
					<view class="fui-tabs__track" :style="[trackStyle]"></view>
				</view>
			</scroll-view>
			<slot name="right"></slot>
		</view>
		<view class="fui-tabs__content"
			@touchstart="onTouchStart"
			@touchmove="onTouchMove"
			@touchend="onTouchEnd"
			@touchcancel="onTouchEnd"
			:class="{'fui-tabs__content--animated': animated}"
			v-if="$slots['default']">
			<view class="fui-tabs__content-inner" :style="[innerStyle]" ref="innerRef">
				<slot></slot>
			</view>  
		</view>
	</view>
...
<script lang="ts">
	...
	import { getRect, getAllRect } from './fui-shared/getRect'
	import { clamp } from './fui-shared/clamp'
		...
		setup(props, {emit}) {
			...
			const moveToActiveTab = () => {
				nextTick(()=>{
					try {
						Promise.all([
							getRect(`.fui-tabs__scroll`, context), 
							getRect(`.fui-tabs__track`, context), 
							getAllRect(`.fui-tabs__item`, context)
			...
			const updateInnerStyle = (offset: number) => {
				...
				nextTick(()=>{
					Promise.all(
						[
							getRect('.fui-tabs__content', context), 
							getRect('.fui-tabs__content-inner', context)
						]).then(([parent, inner])=>{
