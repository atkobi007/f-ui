<template>
	<view class="fui-badge__wrapper" v-if="$slots.default">
		<slot></slot>
		<view v-if="hasContent || props.dot" class="fui-badge" :class="classes" :style="[styles]">
			<slot v-if="$slots.content" name="content"></slot>
			<block v-else-if="renderContent">{{ renderContent }}</block>
		</view>
	</view>
	<view v-else-if="hasContent || props.dot" class="fui-badge" :class="classes" :style="[styles]">
		<slot v-if="$slots.content" name="content"></slot>
		<block v-else-if="renderContent">{{ renderContent }}</block>
	</view>
</template>
<script lang="ts">
	 ...
	 */
	import { computed, defineComponent, getCurrentInstance, onMounted } from '@/uni_modules/f-ui/components/fui-tabs/fui-shared/vue'
	import badgeProps from './props'
	import { isNumeric } from '@/uni_modules/f-ui/components/fui-tabs/fui-shared/isNumeric'
	import { isNumber } from '@/uni_modules/f-ui/components/fui-tabs/fui-shared/isNumber'
	import { addUnit } from '@/uni_modules/f-ui/components/fui-tabs/fui-shared/addUnit'
	import { isDef } from '@/uni_modules/f-ui/components/fui-tabs/fui-shared/isDef'
	import { getClassStr } from '@/uni_modules/f-ui/components/fui-tabs/fui-shared/getClassStr'
	import { getOffsetWithMinusString } from './utils'
	const name = 'fui-badge'
